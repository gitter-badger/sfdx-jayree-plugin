...
exports.postretrieve = async function (options) {
    debug(`called 'postretrieve' by: ${options.Command.id}`);
    ...
    const mdapiFilePaths = Object.values(options.result)
        ...
        .flat();
    const profiles = mdapiFilePaths.filter((el) => el.includes('unpackaged/profiles/'));
    if (profiles.length > 0) {
        const retrievePackageDir = path.join(path.dirname(mdapiFilePaths[0]), '..');
    ...
    const permissionsets = mdapiFilePaths.filter((el) => el.includes('unpackaged/permissionsets/'));
